...
class SequentialRF:
    def __init__(self, dataset, n_folds, model_kwargs, 
        ...
        self.rf_assimilate_1day = RandomForestRegressor(**model_kwargs)
        self.rf_assimilate_2day = RandomForestRegressor(**model_kwargs)
    ...
    def predict(self, X):
        ...
        assim_1d_data, assim_1d_feats, _ = self.make_1d_assimilation_data(X)

        print('\nevaluating assimilation 1-day model')
        assim_1d_preds = self.rf_assimilate_1day.predict(assim_1d_data[assim_1d_feats])
        ...
        self.rf_2day.predict(rf_2d_data[rf_2d_feats])
        
        print('\nevaluating 2-day assimilation model')
        assim_2d_data, assim_2d_feats, _ = self.make_2d_assimilation_data(X, pred_df)

        assim_2d_preds =  self.rf_assimilate_2day.predict(assim_2d_data[assim_2d_feats])
        idxed_1d_preds = pd.DataFrame({'cp' : assim_1d_data.cp, 
                                       'time' : assim_1d_data.time,
                                       'max_frp' : self._denormalize(self.target_col, assim_1d_data.this_calendar_day_max.values),
                                       '1day_pred' : assim_1d_preds})
        idxed_2d_preds = pd.DataFrame({'cp' : assim_2d_data.cp, 
                                       'time' : assim_2d_data.time,
                                       '2day_pred' : assim_2d_preds})
    ...
    def train(self):
        ...
        for fold, (train_idxs, val_idxs) in enumerate(kf.split(self.train_ids)):
            ...
            assim_1d_data, assim_1d_feats, assim_1d_label = self.make_1d_assimilation_data(train_data)

            print('\ntraining assimilation 1-day model')
            self.rf_assimilate_1day.fit(assim_1d_data[assim_1d_feats], 
                                        assim_1d_data[assim_1d_label], 
                                        sample_weight = sample_weights(assim_1d_data[assim_1d_label], 
                                                            factor = self.weight_factor,
                                                            method = self.weight_method))
            ...
            print('\ntraining 2-day assimilation model')
            assim_2d_data, assim_2d_feats, assim_2d_label = self.make_2d_assimilation_data(train_data, pred_df)

            self.rf_assimilate_2day.fit(assim_2d_data[assim_2d_feats], 
                                        assim_2d_data[assim_2d_label], 
                                        sample_weight = sample_weights(assim_2d_data[assim_2d_label], 
                                                factor = self.weight_factor,
                                                method = self.weight_method))
    ...
    def eval(self, fold_num, val_cps):
        ...
        print('\nevaluating assimilation 1-day model')
        assim_1d_data['preds'] = self.rf_assimilate_1day.predict(assim_1d_data[assim_1d_feats])
        ...
        assim_2d_data['preds'] = self.rf_assimilate_2day.predict(assim_2d_data[assim_2d_feats])
        ...
        bucket_preds_1day = self._bin_growth(assim_1d_data, 'preds', 'one_day_pred_bin')[[self.idx_name, 'time', 'one_day_pred_bin']]
        bucket_preds_2day = self._bin_growth(assim_1d_data, 'preds', 'two_day_pred_bin')[[self.idx_name, 'time', 'two_day_pred_bin']]
        ...
        # calculate rmse 
        results_df['rmse_1day'] = root_mean_squared_error(assim_1d_data['preds'].values, 
                                                    assim_1d_data['scaling_label'].values)
        results_df['rmse_2day'] = root_mean_squared_error(assim_2d_data['preds'].values, 
                                                        assim_2d_data['scaling_label'].values)
        
        # calculate adjusted r2 score
        n_1day = len(assim_1d_data['scaling_label'].values)
        n_2day = len(assim_2d_data['scaling_label'].values)
        r2_1day = r2_score(assim_1d_data['preds'].values, 
                            assim_1d_data['scaling_label'].values)
        r2_2day = r2_score(assim_2d_data['preds'].values, 
                            assim_2d_data['scaling_label'].values)
        
        results_df['r2_1day'] = r2_1day
        results_df['r2_2day'] = r2_2day

        results_df['adj_r2_1day'] = 1 - ((1 - r2_1day) * (n_1day - 1)) / (n_1day - len(assim_1d_feats) - 1)
        results_df['adj_r2_2day'] = 1 - ((1 - r2_2day) * (n_2day - 1)) / (n_2day - len(assim_2d_feats) - 1)
    ...
    def process_1d_data(self, X):
        ...
        if self.log_bool:
            df['y_max_growth_24h'] = np.log(future_frp / current_frp)
        else:
    ...
    def process_2d_data(self, X, predictions):
        ...
        if self.log_bool:
            df['y_max_growth_48h'] = np.log(future_frp / np.exp(df['preds']) * current_frp) 
        else:
            df['y_max_growth_48h'] = future_frp / df['preds'] * current_frp
        
        X_features.extend(['yesterday_max_frp', 'pred_interim_max_frp'])
        y_cols = ['y_max_growth_48h']
    ...
    def make_1d_assimilation_data(self, X):
        rf1_df, rf1_X_feats, _ = self.process_1d_data(X)
        rf_1d_preds = self.rf_1day.predict(rf1_df[rf1_X_feats])

        df = rf1_df.copy()
        df['model_output']= rf_1d_preds

        assim_1d_features = []

        # First, find the max FRP for each calendar date per fire
        daily_max = df.groupby([self.idx_name, df['time'].dt.date])[self.target_col].max()

        # Then, map it to the 'tomorrow' date for the current row
        df['tomorrow_date'] = df['time'].dt.date + pd.Timedelta(days=1)
        df['today_date'] = df['time'].dt.date

        df['this_calendar_day_max'] = df.set_index([self.idx_name, 'today_date']).index.map(daily_max)
        df['next_calendar_day_max'] = df.set_index([self.idx_name, 'tomorrow_date']).index.map(daily_max)

        df['this_calendar_day_max'] = df['this_calendar_day_max'].clip(1e-6)
        df['next_calendar_day_max'] = df['next_calendar_day_max'].clip(1e-6)
        assim_1d_features.append('this_calendar_day_max')

        for h in range(1, 25):
            df[f'pred_lag_{h}h'] = df.groupby(self.idx_name)['model_output'].shift(h)
            assim_1d_features.append(f'pred_lag_{h}h')

        # 4. Label: Scaling factor between last max and next day's max
        # Often used to predict growth/decay relative to the current peak
        if self.log_bool:
            df['scaling_label'] = np.log(df['next_calendar_day_max'] / df['this_calendar_day_max'])
        else:
            df['scaling_label'] = df['next_calendar_day_max'] / df['this_calendar_day_max']

        # 5. Filter for your model indices and select your feature columns
        label_col = ['scaling_label']

        final_features = df[df.time.dt.hour == 0].dropna(subset=assim_1d_features + label_col)

        return final_features, sorted(assim_1d_features), label_col

    def make_2d_assimilation_data(self, X, predictions):
        rf2_df, rf2_X_feats, _ = self.process_2d_data(X, predictions)
        rf_2d_preds = self.rf_2day.predict(rf2_df[rf2_X_feats])

        df = rf2_df.copy()
        df['model_output']= rf_2d_preds
        ...
        # Then, map it to the 'tomorrow' date for the current row
        df['next_date'] = df['time'].dt.date + pd.Timedelta(days=2)
        df['today_date'] = df['time'].dt.date

        df['this_calendar_day_max'] = df.set_index([self.idx_name, 'today_date']).index.map(daily_max)
        df['next_calendar_day_max'] = df.set_index([self.idx_name, 'next_date']).index.map(daily_max)

        df['this_calendar_day_max'] = df['this_calendar_day_max'].clip(1e-6)
        df['next_calendar_day_max'] = df['next_calendar_day_max'].clip(1e-6)
        assim_2d_features.append('this_calendar_day_max')

        for h in range(1, 25):
            df[f'pred_lag_{h}h'] = df.groupby(self.idx_name)['model_output'].shift(h)
            assim_2d_features.append(f'pred_lag_{h}h')
        ...
        # 5. Filter for your model indices and select your feature columns
        label_col = ['scaling_label']
        final_features = df[df.time.dt.hour == 0].dropna(subset = assim_2d_features + label_col)

        return final_features, sorted(assim_2d_features), label_col

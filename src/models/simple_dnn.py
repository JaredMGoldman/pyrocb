...
# -----------------------------------------------------------------
# 1. Define the Core DNN Architecture
# -----------------------------------------------------------------
class FireRegressionDNN(nn.Module):
    def __init__(self, input_dim):
        super(FireRegressionDNN, self).__init__()
        # A robust 4-layer fully connected architecture with batch normalization 
        # and dropout to prevent overfitting on complex weather/FRP vectors
        self.network = nn.Sequential(
            ...
            nn.Linear(32, 1)  # Single continuous scaling factor target output
        )
...
class SimpleDNN:
    def __init__(self, dataset, n_folds, model_kwargs, 
                 ...
                 weight_factor = 0.5, weight_method = 'prod'):
        ...
        # Hyperparameters extracted from model_kwargs or defaulted safely
        self.epochs = model_kwargs.get('epochs', 10)
        self.lr = model_kwargs.get('lr', 0.001)
        self.batch_size = model_kwargs.get('batch_size', 128)

        # Models will be instantiated dynamically inside the training loop 
        # once the final processed column shapes are calculated
        self.dnn_1day = None
        self.dnn_2day = None
    ...
    def _pytorch_predict(self, model, X_matrix):
        """Helper to compute neural network evaluation predictions in evaluation mode safely."""
        model.eval()
    ...
    def _pytorch_train_loop(self, model, X_df, y_df, weights_array):
        ...
        # 1. Wrap numpy arrays into a native PyTorch dataset
        ...
        # 2. FIXED: Changed num_workers to 0 to prevent pickling serialization crashes in local class scopes
    ...
    def predict(self, X):
        ...
        print('\nevaluating pred 1-day DNN model')
        ...
        print('\nevaluating pred 2-day DNN model')
    ...
    def train(self):
        ...
        for fold, (train_idxs, val_idxs) in enumerate(kf.split(self.train_ids)):
            ...
            if self.dnn_1day is None:
                self.dnn_1day = FireRegressionDNN(input_dim=len(rf_1d_X_cols))
                
            print(f'\ntraining pred 1-day DNN model [Fold {fold}]')
            ...
            if self.dnn_2day is None:
                self.dnn_2day = FireRegressionDNN(input_dim=len(rf_2d_feats))
                
            print(f'training pred 2-day DNN model [Fold {fold}]')
    ...
    def eval(self, fold_num, val_cps):
        ...
        # Calculate categorical mapping performance accuracy
        ...
        if not cmp_1day_buckets.empty:
            ...
            self._plot_fire_confusion(confusion_1day, f'fold_{fold_num}_1_Day_DNN_Classification_Accuracy', labels)
        if not cmp_2day_buckets.empty:
            ...
            self._plot_fire_confusion(confusion_2day, f'fold_{fold_num}_2_Day_DNN_Classification_Accuracy', labels)
            
        # FIXED: Explicitly casting values to uniform types (float64) before performing evaluations
        ...
        # FIXED: Wrapped calculations inside safe threshold checks to prevent small sample size division by zero errors
    ...
    def save(self):
        """Saves weights for the PyTorch network models successfully."""
        ...
        print(f"Successfully exported PyTorch state tensors to {self.out_dir}")

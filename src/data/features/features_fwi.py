...
def imerg_fwi_timeseries(df, fwi_data_root = "/data/lthapa/data2restore/lthapa"):
    ...
    fire_fwi_intersection_xr = parallel_intersection_labels(df, 'IMERG_FWI_GRID')
    ...
    times = pd.date_range(np.datetime64(df['UTC Day'].iloc[0]),
                        ...
                        np.timedelta64(1,'D'))
    fwi_filenames,times_back_used = make_file_namelist(times,f'{fwi_data_root}/FWI_IMERG/WESTUS_FWI.IMERG.FINAL.v6.Daily.Default.YYYYMMDD.nc')
    ...
    dat_fwi_mean = dat_fwi.resample(time='24H',base=12 ,label='left').mean(dim='time') #take the daily mean         
    ...
    dat_fwi_sub = dat_fwi_mean.sel(lat = fire_fwi_intersection_xr['lat'].values, 
                    ...
                    time = pd.to_datetime(fire_fwi_intersection_xr['12Z Start Day'].values + ' 12:00:00'), method='nearest')#these should be lined up correctly

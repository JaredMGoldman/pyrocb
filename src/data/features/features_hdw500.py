...
from utils import calculate_intersection, make_file_namelist, generate_df, ML_DATA_ROOT, get_repo_root


def hdw500_lagged_timeseries(df, hrrr_data_root = f"{ML_DATA_ROOT}/pygraf/processed_hrrr_hdw_hwp"):  #with the wind speed
    ...
    files_all, times_all_used = make_file_namelist(times_back_all, f"{hrrr_data_root}/Processed_HRRR_YYYYMMDDHH_HDW500.nc")
    ...
    for i, today in enumerate(todays):
        ...
        for day_num in range(4):
            ...
            hdw0_daily_mean = (hd * w0).resample(time="24h", offset="12h", label="left").mean()

...
def hwp_timeseries(df, hrrr_data_root = '/data2/lthapa/ML_daily/pygraf/processed_hrrr_hdw_hwp'):  #with the wind speed
    ...
    for i, today in enumerate(times_intersect):
        ...
        df_hwp_weighted.loc[i, ('hwp')] =np.nansum((hrrr_daily_mean_region['hwp'].sel(time=np.datetime64(today+ ' 12:00:00')).values)*(intersection_sub['weights'].values))  
        ...
        df_hwp_unweighted.loc[i, ('hwp')] = np.nanmean((hrrr_daily_mean_region['hwp'].sel(time=np.datetime64(today+ ' 12:00:00')).values)*(intersection_sub['weights_mask'].values))
